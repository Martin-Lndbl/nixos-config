...
in
{
  ...
  # regenerated by caelestia on every scheme change; hyprland.nix dofile()s it
  # at startup so the active border colour tracks the live wallpaper theme
  xdg.configFile."caelestia/templates/caelestia-colors.lua".text = ''
    return {
      active = "rgba({{ primary.hex }}ff)"
    }
  '';
